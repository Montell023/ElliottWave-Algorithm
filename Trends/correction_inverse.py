...
def correction_inverse(data, peak_mask, valley_mask):
    ...
    # Identify the first peak as the starting point
    starting_point = data.loc[peaks[0]]

    # Identify Wave A
    for valley in valleys:  # Iterate in natural order
        if valley not in data.index:
            continue
        if data.loc[valley] < data.mean() and data.loc[valley] < starting_point:
            waves['Wave A'] = valley
            break
    ...
    for peak in peaks:
        if peak not in data.index:
            continue
        if data.loc[peak] > data.loc[waves['Wave A']] and data.loc[peak] < starting_point:
            ...
            if (data.loc[peak] > fib_retracements).any() or len(corrective_waves) > 0 or len(subdivides_downtrend_waves) > 0:
                waves['Wave B'] = peak
                break
    ...
    for valley in valleys:  # Iterate in natural order
        if valley not in data.index:
            continue
        if data.loc[valley] < data.mean() and data.loc[valley] < data.loc[waves['Wave B']]:
            ...
            if (data.loc[valley] < fib_extensions).any() or len(motive_waves) > 0 or len(subdivides_waves) > 0:
                waves['Wave C'] = valley
                break
