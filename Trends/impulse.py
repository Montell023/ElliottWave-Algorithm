...
def uptrend(data, mean_price, lower_band, above_mean, peaks, valleys):
    ...
    for i in range(len(data)):
        ...
        if starting_point is None and valleys.iloc[i] == lower_band.iloc[i] and price < mean_price.iloc[i]:
            classifications.append('Starting Point')
            starting_point = price

        elif starting_point is not None and wave_1 is None and price > starting_point and price > mean_price.iloc[i] and peaks.iloc[i]:
            motive_waves = motive(peaks, valleys)
            subdivide_waves = subdivides(peaks, valleys)
            if motive_waves or subdivide_waves:
                classifications.append('Wave 1')
                wave_1 = price

        elif wave_1 is not None and wave_2 is None and price > starting_point and price < wave_1 and price < mean_price.iloc[i] and valleys.iloc[i]:
        ...
        elif wave_2 is not None and wave_3 is None and price > mean_price.iloc[i] and (price > wave_1 * 1.618 or price > wave_1 * 2.618 or price > wave_1 * 4.236) and peaks.iloc[i]:
            motive_waves = motive(peaks, valleys)
            subdivide_waves = subdivides(peaks, valleys)
            if motive_waves or subdivide_waves:
        ...
        elif wave_3 is not None and wave_4 is None and price < wave_3 and price < mean_price.iloc[i] and price > wave_1 and valleys.iloc[i]:
        ...
        elif wave_4 is not None and wave_5 is None and price > mean_price.iloc[i] and price > wave_3 and price == above_mean.iloc[i] and peaks.iloc[i]:
            motive_waves = motive(peaks, valleys)
            subdivide_waves = subdivides(peaks, valleys)
            if motive_waves or subdivide_waves:
        ...
        elif wave_4 is not None and wave_5 is None:
            expected_wave_5 = wave_4 + (wave_1 - starting_point)
            if price >= expected_wave_5 and peaks.iloc[i]:
                classifications.append('Wave 5')
                wave_5 = price
            else:
